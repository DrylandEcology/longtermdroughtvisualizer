...
set_execute_SW <- function(lat, lng, futuresim,
  ...
  soils_info <- Soils[,c('depth_cm', 'sand_frac', 'clay_frac')]
  soils_info$width <- diff(c(0, Soils$depth_cm))
  soils_info <- merge(soils_info, soils_info0, by = 'depth_cm')
  soils_info$variable <- paste0('Lyr_',1:dim(SoilsDF)[1])
  
  # get weighted average of sand and clay
  soils_info_avg <- setDT(soils_info)[,.(sand = weighted.mean(sand_frac, width),
  ...
  if(futuresim == 1) {
    ...
    toEXPORT <- 'get_MACA_one_scenario'
      
    AllVars <- foreach::foreach(g = 1:8,
                                .combine = rbind,
                                .export = toEXPORT,
                                .packages = c('lubridate', "data.table")) %dopar%
      
      {
        weath <- get_MACA_one_scenario(lat = lat, lng = lng,
